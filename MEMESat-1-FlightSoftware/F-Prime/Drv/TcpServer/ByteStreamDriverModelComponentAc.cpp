...
#include <Drv/TcpServer/ByteStreamDriverModelComponentAc.hpp>
...
namespace Drv {
    ...
    init(NATIVE_INT_TYPE instance)
  {
    ...
    for (
        ...
        port < this->getNum_poll_InputPorts();
        port++
    ) {
    ...
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_send_InputPorts();
        port++
    ) {
    ...
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_allocate_OutputPorts();
        port++
    ) {
    ...
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_deallocate_OutputPorts();
        port++
    ) {
    ...
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_ready_OutputPorts();
        port++
    ) {
    ...
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_recv_OutputPorts();
        port++
    ) {
