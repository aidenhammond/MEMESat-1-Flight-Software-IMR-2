...
namespace Svc {
  ...
  // ----------------------------------------------------------------------
    ...
    FileUplinkComponentBase :: FileUplinkComponentBase(const char* compName) :
  ...
  void FileUplinkComponentBase ::
    tlmWrite_FilesReceived(U32 arg, Fw::Time _tlmTime)
  {
  ...
  void FileUplinkComponentBase ::
    tlmWrite_PacketsReceived(U32 arg, Fw::Time _tlmTime)
  {
    if (this->m_tlmOut_OutputPort[0].isConnected()) {
      if (this->m_timeCaller_OutputPort[0].isConnected() && _tlmTime ==  Fw::ZERO_TIME) {
         this->m_timeCaller_OutputPort[0].invoke( _tlmTime);
  ...
  void FileUplinkComponentBase ::
    log_WARNING_HI_BadChecksum(
        const Fw::LogStringArg& fileName, U32 computed, U32 read
    )
  {
    ...
    if (this->m_eventOut_OutputPort[0].isConnected()) {
      ...
      _status = fileName.serialize(_logBuff, 40);
      FW_ASSERT(
          _status == Fw::FW_SERIALIZE_OK,
          static_cast<AssertArg>(_status)
      );
    ...
    if (this->m_LogText_OutputPort[0].isConnected()) {
      ...
      const char* _formatString =
        "(%s) %s: Bad checksum value during receipt of file %s: computed 0x%x, read 0x%x";
#else
      const char* _formatString =
        "%s: Bad checksum value during receipt of file %s: computed 0x%x, read 0x%x";
#endif
    ...
    )
  {
    ...
    void FileUplinkComponentBase::log_WARNING_HI_FileWriteError_ThrottleClear() {
    ...
    )
  {
    ...
    void FileUplinkComponentBase::log_WARNING_HI_InvalidReceiveMode_ThrottleClear() {
        // reset throttle counter
        this->m_InvalidReceiveModeThrottle = 0;
    }
  ...
  void FileUplinkComponentBase ::
    log_WARNING_HI_PacketOutOfBounds(
        U32 packetIndex, const Fw::LogStringArg& fileName
    )
  {
    ...
    void FileUplinkComponentBase::log_WARNING_HI_PacketOutOfBounds_ThrottleClear() {
        // reset throttle counter
        this->m_PacketOutOfBoundsThrottle = 0;
    }
    ...
    )
  {
    ...
    void FileUplinkComponentBase::log_WARNING_HI_PacketOutOfOrder_ThrottleClear() {
        // reset throttle counter
        this->m_PacketOutOfOrderThrottle = 0;
    }
...
#if FW_OBJECT_NAMES == 1
      const char* _formatString =
        "(%s) %s: Unable to decode file packet. Status: %d";
#else
      const char* _formatString =
        "%s: Unable to decode file packet. Status: %d";
#endif
