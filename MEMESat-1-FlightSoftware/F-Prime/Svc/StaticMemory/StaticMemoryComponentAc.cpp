...
namespace Svc {
  ...
  {
    FW_ASSERT(portNum < this->getNum_bufferAllocate_InputPorts(),static_cast<AssertArg>(portNum));
  ...
  {
    FW_ASSERT(portNum < this->getNum_bufferDeallocate_InputPorts(),static_cast<AssertArg>(portNum));
    ...
    init(NATIVE_INT_TYPE instance)
  {
    ...
    for (
        ...
        port < this->getNum_bufferAllocate_InputPorts();
        port++
    ) {
    ...
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_bufferDeallocate_InputPorts();
        port++
    ) {
      ...
      (void) snprintf(
          portName,
          sizeof(portName),
          "%s_bufferDeallocate_InputPort[%d]",
          this->m_objName,
          port
      );
