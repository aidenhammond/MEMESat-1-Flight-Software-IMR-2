...
namespace Svc {
  ...
  {
    FW_ASSERT(portNum < this->getNum_SendFile_InputPorts(),static_cast<AssertArg>(portNum));
  ...
  {
    FW_ASSERT(portNum < this->getNum_bufferReturn_InputPorts(),static_cast<AssertArg>(portNum));
  ...
  {
    FW_ASSERT(portNum < this->getNum_FileComplete_OutputPorts(),static_cast<AssertArg>(portNum));
    ...
    )
  {
    ...
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_SendFile_InputPorts();
        port++
    ) {
    ...
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_bufferReturn_InputPorts();
        port++
    ) {
    ...
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_FileComplete_OutputPorts();
        port++
    ) {
