...
namespace Svc {
  ...
  {
    FW_ASSERT(portNum < this->getNum_framedIn_InputPorts(),static_cast<AssertArg>(portNum));
  ...
  {
    FW_ASSERT(portNum < this->getNum_bufferAllocate_OutputPorts(),static_cast<AssertArg>(portNum));
  ...
  {
    FW_ASSERT(portNum < this->getNum_framedDeallocate_OutputPorts(),static_cast<AssertArg>(portNum));
  ...
  {
    FW_ASSERT(portNum < this->getNum_framedPoll_OutputPorts(),static_cast<AssertArg>(portNum));
    ...
    init(NATIVE_INT_TYPE instance)
  {
    ...
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_framedIn_InputPorts();
        port++
    ) {
    ...
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_bufferAllocate_OutputPorts();
        port++
    ) {
    ...
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_framedDeallocate_OutputPorts();
        port++
    ) {
      ...
      (void) snprintf(
          portName,
          sizeof(portName),
          "%s_framedDeallocate_OutputPort[%d]",
          this->m_objName,
          port
      );
    ...
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_framedPoll_OutputPorts();
        port++
    ) {
