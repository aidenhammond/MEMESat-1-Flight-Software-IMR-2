...
namespace Svc {
  ...
  // ----------------------------------------------------------------------
    ...
    HealthComponentBase :: HealthComponentBase(const char* compName) :
    ...
    )
  {
    ...
    switch (opCode - idBase) {
      ...
      case OPCODE_HLTH_ENABLE: /*!< A command to enable or disable health checks */
      ...
      case OPCODE_HLTH_PING_ENABLE: /*!< Ignore a particular ping entry */
      ...
      case OPCODE_HLTH_CHNG_PING: /*!< Change ping value */
  ...
  void HealthComponentBase ::
    tlmWrite_PingLateWarnings(U32 arg, Fw::Time _tlmTime)
  {
  ...
  void HealthComponentBase ::
    log_FATAL_HLTH_PING_WRONG_KEY(
        const Fw::LogStringArg& entry, U32 badKey
    )
  {
...
#if FW_OBJECT_NAMES == 1
      const char* _formatString =
        "(%s) %s: Ping entry %s responded with wrong key 0x%x";
#else
      const char* _formatString =
        "%s: Ping entry %s responded with wrong key 0x%x";
#endif
  ...
  void HealthComponentBase ::
    log_ACTIVITY_HI_HLTH_CHECK_PING(
        Fw::Enabled enabled, const Fw::LogStringArg& entry
    )
  {
  ...
  void HealthComponentBase ::
    log_ACTIVITY_HI_HLTH_PING_UPDATED(
        const Fw::LogStringArg& entry, U32 warn, U32 fatal
    )
  {
...
#if FW_OBJECT_NAMES == 1
      const char* _formatString =
        "(%s) %s: Health ping for %s changed to WARN %u FATAL %u";
#else
      const char* _formatString =
        "%s: Health ping for %s changed to WARN %u FATAL %u";
#endif
...
#if FW_OBJECT_NAMES == 1
      const char* _formatString =
        "(%s) %s: Health ping for %s invalid values: WARN %u FATAL %u";
#else
      const char* _formatString =
        "%s: Health ping for %s invalid values: WARN %u FATAL %u";
#endif
