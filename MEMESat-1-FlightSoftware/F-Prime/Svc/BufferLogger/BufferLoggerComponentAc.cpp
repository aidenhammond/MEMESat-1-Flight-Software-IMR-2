...
namespace Svc {
  ...
  {
    FW_ASSERT(portNum < this->getNum_eventOutText_OutputPorts(),static_cast<AssertArg>(portNum));
    ...
    )
  {
    ...
    for (
        NATIVE_INT_TYPE port = 0;
        port < this->getNum_eventOutText_OutputPorts();
        port++
    ) {
