module Ref {
  ...
  module Default {
    ...
    # Changed 1024 -> 8192 due to warning that stack
    # size was too small
    constant stackSize = 16 * 8192
  ...
  # Changed priority from 101 -> 99
  instance cmdDisp: Svc.CommandDispatcher base id 0x0500 \
    queue size 20 \
    stack size Default.stackSize \
    priority 99

  # Changed priority from 100 -> 99
  ...
  # Changed priority from 101 -> 99
  instance FileRecycler: Ref.FileRecycler base id 0x0ED0 \
    queue size Default.queueSize \
    stack size Default.stackSize \
    priority 99

  # Changed priority from 101 -> 99
  instance Scheduler: Ref.Scheduler base id 0xA000 \
    queue size Default.queueSize \
    stack size Default.stackSize \
    priority 99

  # Changed priority from 101 -> 99
  instance TlmChanWrapper: Ref.TlmChanWrapper base id 0xA0F0 \
    queue size Default.queueSize \
    stack size Default.stackSize \
    priority 99
    
  # Changed priority from 101 -> 99
  instance StateMachine: Ref.StateMachine base id 0xAD00 \
    queue size Default.queueSize \
    stack size Default.stackSize \
    priority 99 

  # Changed priority from 101 -> 99
  instance eps_uart: Ref.EPS_UART base id 0x0E00 \
    queue size Default.queueSize \
    stack size Default.stackSize \
    priority 99
  ...
  instance uartDrv: Drv.LinuxSerialDriver base id 0x04B00 \
  {

    phase Fpp.ToCpp.Phases.configComponents """
    {
      const bool status = uartDrv.open("/dev/ttyS0",
